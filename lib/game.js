...
const handleServicePayments = (userData, companyData) => {
    ...
    if (sufficientFunds(servicePrice, userFunds)) {
        updatePlayerFunds(userData, companyData)
    } else {
...
const updatePlayerFunds = (userData, companyData) => {
    topPlayerUp(companyData)
    takeRentFromPlayer(userData, companyData)
    //Get amount of rent to pay
    //Get user
    //Patch request to payer 
    console.log("pay me brother")
}

const topPlayerUp = (companyData) => {
    companyData.player.money = companyData.player.money + (companyData.price * 0.1)
    API.update(`${PLAYER_URL}/${companyData.player.id}`, companyData.player)
}

const takeRentFromPlayer = (userData, companyData) => {
    userData.money += userData.money - (companyData.price * 0.1)
